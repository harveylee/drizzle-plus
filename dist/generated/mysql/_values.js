...
* ```
*/
MySqlDatabase.prototype.$values = function(rows, typings) {
...
MySqlDatabase.prototype.$withValues = function(alias, rows, typings) {
	const withSubquery = createWithSubquery(this.$values(rows, typings).getSQL(), alias, new Proxy(rows[0], { get: (_, key) => is(typings, MySqlTable) && getTableColumns(typings)[key] || noopDecoder$1 }));
...
var ValuesList = class {
	...
	constructor(casing, keys, rows, typings) {
		...
		this.keys = keys;
		this.rows = rows;
		this.typings = typings && (is(typings, MySqlTable) ? getTableColumns(typings) : typings);
	}
	as(alias) {
		...
		this.keys.forEach((key, index) => {
			...
			if (is(type, MySqlColumn)) field.mapWith(type);
		});
		...
		for (let rowIndex = 0; rowIndex < this.rows.length; rowIndex++) {
			...
			this.keys.forEach((key, keyIndex) => {
				...
				if (rowIndex === 0) {
					...
					if (type) value = sql$1`cast(${value} as ${sql$1.raw(is(type, MySqlColumn) ? type.getSQLType() : type)})`;
				}
				chunks.push(value);
